...
inline double l(std::pair <std::vector<double>, double> ds_item, const std::vector<double> &w) {
...
double sgd(DDDataSet ds,double delta,double epsilon,std::vector<double> * _w,
    ...
    while((m_ls = lc(ds, w)) > epsilon && c_step < step_limit) {
        ...
        for(unsigned i = 0 ; i < ds.size() ; i ++ ) {
            ...
            {
                w[j] +=  delta * ( ds.Yat(i) - fx ) * ( j > 0 ? ds.Xat(i,j-1) : 1 );
            }
        }
    }
    return m_ls;
...
double bgd(DDDataSet ds,double delta,double epsilon,std::vector<double> * _w,
        uint64_t step_limit,bool dynamd) {
    return 999.9999;  // not implemented , ref : https://github.com/yujing5b5d/GradientDescent/blob/master/main.cc
}
